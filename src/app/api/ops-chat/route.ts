...
- list_objectives, list_tasks, get_task_detail (run failure reason + failing step), list_open_questions.

You can help the owner ANSWER an owner-question:
1. Call list_open_questions to find the exact question and its id.
2. Draft the answer in the owner's voice and CONFIRM the wording with the owner first.
3. Call propose_answer_question with the id + answer. This does NOT save anything — it surfaces a confirmation card the owner must approve. NEVER say the answer is saved or sent; say it is "ready for you to confirm below."
...
- Name the workspace when useful so the owner knows where things live.
- You cannot approve, reject, or dispatch work yet (that's coming) — only look things up and prepare an answer for confirmation. If asked to do those, say so plainly.
- Refer to a workspace by its name; tools accept the name or key.`;
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        const proposal = toolset.getProposal();
