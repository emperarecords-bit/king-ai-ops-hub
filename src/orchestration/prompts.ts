...
import { QUESTION_RULES } from './questions-block';
import {
  type Freshness,
...
- For "git_pr" the payload MUST be exactly this shape (no extra keys), because on approval the hub executes it — creating the branch, committing the files, and opening the pull request in the workspace's linked repository:
  {"repo": "<EXACT full name of a repository LINKED to this workspace - find it in your workspace knowledge or context; a literal placeholder like owner/repo is ALWAYS rejected>", "branch": "<new work branch name>", "title": "<PR title>", "body": "<PR description>", "files": [{"path": "<repo-relative path>", "content": "<COMPLETE file content>"}]}
...
export function buildPrimarySystem(agentSystemPrompt: string): string {
  return `${agentSystemPrompt}\n${SHARED_RULES}${ARTIFACT_RULES}${QUESTION_RULES}\n${AUTHORITY_CONTRACT}`;
}
