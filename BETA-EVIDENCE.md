...
- **Root cause (if known):** The review step's context omits the Decision Memory (accepted decisions) the primary was given, so the reviewer can't distinguish legitimate memory-grounding from a hallucinated appeal to authority.
- **Resolution:** None (measuring). Candidate **smallest** fix *if confirmed recurring*: include an accepted-decision summary (or a "Decision Memory is authoritative context" note) in the review step's prompt so it can validate memory-grounded claims. Contained context-plumbing change — not a retrieval/architecture rebuild.
- **Follow-up required:** **This is the leading candidate for the first evidence-justified product change.** Confirm recurrence: run 1–2 more memory-grounded tasks and check whether the reviewer again penalizes correct grounding / forces authority-diluting revisions. If it recurs, propose the smallest review-context fix.
